...
-- ============================================
-- 7. quick_consultations 테이블 생성 (빠른 상담용)
-- ============================================
CREATE TABLE IF NOT EXISTS quick_consultations (
  ...
  phone TEXT NOT NULL,
  agree_privacy BOOLEAN DEFAULT false NOT NULL,
  source TEXT DEFAULT 'quick-bar',  -- 어느 페이지에서 신청했는지
  status TEXT DEFAULT 'pending' NOT NULL CHECK (status IN ('pending', 'contacted', 'completed', 'cancelled')),
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  updated_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
);
...
-- 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_quick_consultations_status
  ON quick_consultations(status);

CREATE INDEX IF NOT EXISTS idx_quick_consultations_created_at
  ON quick_consultations(created_at DESC);

-- updated_at 트리거
DROP TRIGGER IF EXISTS update_quick_consultations_updated_at ON quick_consultations;

CREATE TRIGGER update_quick_consultations_updated_at
  BEFORE UPDATE ON quick_consultations
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();
...
-- RLS 정책
DROP POLICY IF EXISTS "Anyone can insert quick_consultations" ON quick_consultations;

CREATE POLICY "Anyone can insert quick_consultations"
  ON quick_consultations
  FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Authenticated users can view quick_consultations" ON quick_consultations;

CREATE POLICY "Authenticated users can view quick_consultations"
  ON quick_consultations
  FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can update quick_consultations" ON quick_consultations;

CREATE POLICY "Authenticated users can update quick_consultations"
  ON quick_consultations
  FOR UPDATE
  TO authenticated
  USING (true)
