...
import { buildHreflangMap, generateReviewsAggregateSchema, safeJsonLd } from '@/lib/seo';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const locales = await getIndexableReviewLocales();
  const indexable = locales.includes(locale);
  return {
    ...base,
    alternates: {
      canonical: base.alternates?.canonical,
      ...(indexable ? { languages: buildHreflangMap('/reviews', locales) } : {}),
    },
