...
export default function UltheraLayout({
  children,
}: {
  ...
  const pageSchema = generateWebPageSchema({
    ...
    breadcrumbs: [
      ...
      { name: treatment.name, url: '/lifting/ulthera' },
    ],
  });
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: `${treatment.name} | ${SITE_INFO.name}`,
    description: treatment.description,
    keywords: [treatment.name, treatment.nameEn, '리프팅', 'HIFU', '울쎄라', '비수술 리프팅', '신사역 피부과'],
    openGraph: {
      title: `${treatment.name} | ${SITE_INFO.name}`,
      description: treatment.shortDesc,
      url: `${BASE_URL}/${locale}/lifting/ulthera`,
      siteName: SITE_INFO.name,
      type: 'website',
    },
    alternates: {
      canonical: `${BASE_URL}/${locale}/lifting/ulthera`,
      languages: buildHreflangMap('/lifting/ulthera'),
    },
  };
}
