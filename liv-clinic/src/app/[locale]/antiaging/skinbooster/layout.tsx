...
export default function SkinboosterLayout({
  children,
}: {
  ...
  const pageSchema = generateWebPageSchema({
    ...
    breadcrumbs: [
      ...
      { name: treatment.name, url: '/antiaging/skinbooster' },
    ],
  });
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: `${treatment.name} | ${SITE_INFO.name}`,
    description: treatment.description,
    keywords: [treatment.name, treatment.nameEn, '스킨부스터', '리쥬란', '쥬베룩', '물광주사', '신사역 피부과'],
    openGraph: {
      title: `${treatment.name} | ${SITE_INFO.name}`,
      description: treatment.shortDesc,
      url: `${BASE_URL}/${locale}/antiaging/skinbooster`,
      siteName: SITE_INFO.name,
      type: 'website',
    },
    alternates: {
      canonical: `${BASE_URL}/${locale}/antiaging/skinbooster`,
      languages: buildHreflangMap('/antiaging/skinbooster'),
    },
  };
}
