...
// 시술 데이터 가져오기
const treatment = TREATMENTS.antiaging.skincare;
...
export default function SkincareLayout({
  ...
  const pageSchema = generateWebPageSchema({
    ...
    breadcrumbs: [
      ...
      { name: treatment.name, url: '/antiaging/skincare' },
    ],
  });
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: `${treatment.name} | ${SITE_INFO.name}`,
    description: treatment.description,
    keywords: [treatment.name, treatment.nameEn, '물톡스', '플라필', '피부관리', '스킨케어', '신사역 피부관리'],
    openGraph: {
      title: `${treatment.name} | ${SITE_INFO.name}`,
      description: treatment.shortDesc,
      url: `${BASE_URL}/${locale}/antiaging/skincare`,
      siteName: SITE_INFO.name,
      type: 'website',
    },
    alternates: {
      canonical: `${BASE_URL}/${locale}/antiaging/skincare`,
      languages: buildHreflangMap('/antiaging/skincare'),
    },
  };
}
