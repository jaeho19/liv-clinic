...
const GUIDE_SEARCH_INDEX: GuideSearchEntry[] = [
  // ── 시스템 개요 ──
  { id: 'overview-access', sectionId: 'overview', type: 'overview', title: '접속 방법',
    content: '브라우저에서 관리자 페이지 URL/admin 접속 → 이메일/비밀번호 로그인 → 좌측 사이드바에서 메뉴 선택',
    keywords: ['접속', '로그인', '브라우저', 'URL', '사이드바', '시작', '처음'] },
  { id: 'overview-logout', sectionId: 'overview', type: 'overview', title: '로그아웃 방법',
    content: 'PC: 좌측 사이드바 하단 로그아웃 버튼. 모바일: ☰ 메뉴 열기 → 하단 로그아웃 버튼',
    keywords: ['로그아웃', '로그 아웃', '나가기', '종료', '퇴근'] },
  { id: 'overview-pwa', sectionId: 'overview', type: 'tip', title: '모바일에서 앱처럼 사용하기',
    content: 'Chrome > 메뉴 > "홈 화면에 추가" 또는 Safari > 공유 > "홈 화면에 추가"를 하면 앱 아이콘처럼 바로 접속 가능',
    keywords: ['앱', '설치', 'PWA', '홈화면', '홈 화면', '아이콘', '모바일'] },
  // ── 대시보드 ──
  { id: 'dashboard-usage', sectionId: 'dashboard', type: 'scenario', title: '출근 후 당일 현황 확인',
    content: '로그인 시 자동 표시. 통계 카드 클릭으로 해당 페이지 이동. 오늘의 콜백/알림 예정 확인',
    keywords: ['대시보드', '현황', '통계', '출근', '오늘', '카드', '7일', '추이'] },
  // ── 상담관리 ──
  { id: 'consult-new', sectionId: 'consultations', type: 'scenario', title: '신규 상담 접수 후 처리',
    content: '신규 탭에서 확인 → 상세 열기 → 담당자 배정 → 상태 변경(콜백예정/예약확정) → 메모 작성(음성 가능)',
    keywords: ['신규', '상담', '접수', '담당자', '배정', '상태', '변경', '메모'] },
  { id: 'consult-callback', sectionId: 'consultations', type: 'scenario', title: '콜백 처리',
    content: '콜백예정 탭 → "오늘 콜백만" 체크 → 전화 후 팔로업 결과 기록 → 다음 콜백 설정 또는 예약확정',
    keywords: ['콜백', '팔로업', '전화', '재연락', '콜백예정', '오늘'] },
  { id: 'consult-bulk', sectionId: 'consultations', type: 'scenario', title: '벌크(일괄) 처리',
    content: '체크박스로 여러 건 선택 → 상단 벌크 액션 바에서 상태 일괄 변경 또는 담당자 일괄 배정',
    keywords: ['벌크', '일괄', '한번에', '여러', '선택', '일괄변경'] },
  { id: 'consult-features', sectionId: 'consultations', type: 'feature', title: '상담관리 주요 기능',
    content: '상태 탭 필터(9개), 검색/필터, 인라인 편집, 음성 메모, 타임라인, 벌크 액션, CSV 다운로드, 실시간 알림',
    keywords: ['상담', '필터', '검색', '인라인', '편집', '타임라인', 'CSV', '다운로드', '알림'] },
  // ── 운영현황 ──
  { id: 'ops-new-case', sectionId: 'operations', type: 'scenario', title: '환자 입실 시 케이스 등록',
    content: '+ 새 케이스 버튼 → 수동/음성 입력 → 환자명, 시술, 담당의, 소요시간 입력 → 대기실에 케이스 생성',
    keywords: ['케이스', '등록', '입실', '환자', '새 케이스', '시술실', '운영'] },
  { id: 'ops-move-complete', sectionId: 'operations', type: 'scenario', title: '시술실 이동 및 완료 처리',
    content: '평면도에서 방 클릭 → 대기 케이스 시작 → 시술 완료 버튼(소요시간 자동 기록)',
    keywords: ['시술실', '이동', '완료', '평면도', '방', '시작', '소요시간'] },
  { id: 'ops-features', sectionId: 'operations', type: 'feature', title: '운영현황 주요 기능',
    content: '평면도(시술실 배치), 칸반보드(5단계), 통계 배지, 음성 입력, 소요시간 자동 업데이트',
    keywords: ['평면도', '칸반', '보드', '배치', '통계', '음성', '입력', '운영현황'] },
  // ── 재고관리 ──
  { id: 'inv-use', sectionId: 'inventory', type: 'scenario', title: '시술 후 소모품 사용 기록',
    content: '사용 기록 버튼 → 시술 레시피 선택(자동 채움) → 사용량 확인/조정 → 저장(재고 자동 차감)',
    keywords: ['사용', '기록', '소모품', '레시피', '차감', '재고', '시술'] },
  { id: 'inv-restock', sectionId: 'inventory', type: 'scenario', title: '부족 재고 입고 처리',
    content: '부족 재고 알림 배너 확인 → 품목 클릭(상세 패널) → 입고 버튼 → 수량/메모 입력',
    keywords: ['입고', '부족', '소진', '재고', '주문', '알림', '배너', '공급'] },
  { id: 'inv-features', sectionId: 'inventory', type: 'feature', title: '재고관리 주요 기능',
    content: '3개 탭(현황/사용이력/입고), 3가지 뷰(테이블/카드/그룹), 검색, 시술 레시피, 소진 예측',
    keywords: ['재고', '뷰', '테이블', '카드', '그룹', '카테고리', '예측', '소진'] },
  // ── 알림관리 ──
  { id: 'noti-send', sectionId: 'notifications', type: 'scenario', title: '시술 후 재방문 알림 발송',
    content: '시술 기록 추가 → 알림 주기 설정(보톡스 90일 등) → KPI에서 발송 대상 확인 → 채널(카카오/SMS) 선택 → 실제 발송',
    keywords: ['알림', '발송', '재방문', '카카오톡', 'SMS', '문자', '주기', '시술기록'] },
  { id: 'noti-history', sectionId: 'notifications', type: 'scenario', title: '발송 이력 확인',
    content: '발송 이력 버튼 → 채널/상태/기간 필터 → 이력 테이블 확인 → CSV 내보내기(선택)',
    keywords: ['발송', '이력', '히스토리', '내역', '확인', 'CSV'] },
  { id: 'noti-template', sectionId: 'notifications', type: 'scenario', title: '알림 템플릿 관리',
    content: '템플릿 버튼 → 새 템플릿 추가(시술명/유형/메시지) → {name},{treatment},{days},{event} 변수 활용',
    keywords: ['템플릿', '메시지', '변수', '작성', '알림문구', '영상'] },
  // ── 리포트 ──
  { id: 'report-monthly', sectionId: 'reports', type: 'scenario', title: '월간 실적 분석',
    content: '년/월 선택 → 전월 비교 체크 → 핵심 지표(매출/전환율/시술수) → 퍼널, 일별 추이, 시술별/의사별 성과',
    keywords: ['리포트', '실적', '분석', '매출', '전환율', '월간', '통계', '의사별', '퍼널'] },
  // ── 매출관리 ──
  { id: 'rev-payment', sectionId: 'revenue', type: 'scenario', title: '시술 후 결제 처리',
    content: '미결제 탭에서 대기 건 확인 → 결제처리 버튼 → 금액/방법(카드/현금/이체/할부) → 결제완료',
    keywords: ['결제', '카드', '현금', '이체', '할부', '미결제', '수납'] },
  { id: 'rev-refund', sectionId: 'revenue', type: 'scenario', title: '환불 처리',
    content: '결제완료 건에서 환불 버튼 클릭 → 확인 팝업에서 환불 선택 → 상태가 환불로 변경',
    keywords: ['환불', '취소', '반환', '돌려', '환불처리'] },
  { id: 'rev-features', sectionId: 'revenue', type: 'feature', title: '매출관리 주요 기능',
    content: '기간 필터(오늘/이번주/이번달), 상태 탭(전체/결제완료/미결제/환불), 7일 추이, CSV 가져오기/내보내기',
    keywords: ['매출', 'CSV', '가져오기', '내보내기', '추이', '기간'] },
  // ── 환자조회 ──
  { id: 'patient-search', sectionId: 'patients', type: 'scenario', title: '재방문 환자 이력 확인',
    content: '이름/전화번호 검색(2글자 이상) → 환자 선택 → 시술이력/상담이력/알림이력/매출분석 4개 탭',
    keywords: ['환자', '검색', '이력', '프로필', '재방문', '조회', '전화번호', '이름'] },
  // ── 음성 노트 ──
  { id: 'voice-ops', sectionId: 'voice-note', type: 'scenario', title: '모바일에서 운영현황 음성 등록',
    content: '운영 현황 템플릿 선택 → 마이크 → 환자명/방번호/시술명/담당의 말하기 → 폼 검토 → 케이스 추가',
    keywords: ['음성', '모바일', '운영', '마이크', '말하기', '녹음', '음성입력'] },
  { id: 'voice-quick', sectionId: 'voice-note', type: 'scenario', title: '빠른 메모 (퀵노트/자유입력)',
    content: '퀵노트(대상/내용/긴급도 3항목) 또는 자유입력 선택 → 마이크로 내용 입력 → 저장',
    keywords: ['퀵노트', '메모', '자유', '빠른', '기록', '노트'] },
  { id: 'voice-tips', sectionId: 'voice-note', type: 'tip', title: '음성 인식 팁',
    content: '조용한 환경, 짧고 명확하게 한 항목씩, 시술명 정확히, 숫자는 단위와 함께. iOS Chrome 부분 지원',
    keywords: ['음성', '인식', '팁', '안돼', '안 돼', '안되', '호환', '브라우저', 'iOS', 'Safari', 'Chrome'] },
  // ── 이벤트관리 ──
  { id: 'event-create', sectionId: 'events', type: 'scenario', title: '새 이벤트 등록',
    content: '새 이벤트 버튼 → 제목/카테고리/기간/설명/포스터 입력 → 저장(즉시 게시) 또는 임시저장(Draft)',
    keywords: ['이벤트', '등록', '새', '만들기', '프로모션', '포스터', '게시'] },
  { id: 'event-features', sectionId: 'events', type: 'feature', title: '이벤트관리 주요 기능',
    content: '상태 관리(활성/종료/임시저장), 검색, 복제, 삭제(홈페이지 즉시 반영)',
    keywords: ['이벤트', '복제', '삭제', '상태', '활성', '종료'] },
  // ── 팝업관리 ──
  { id: 'popup-create', sectionId: 'popups', type: 'scenario', title: '팝업 등록 및 활성화',
    content: '새 팝업 버튼 → 제목/이미지/링크/기간/모바일 표시 여부 → 토글로 활성/비활성',
    keywords: ['팝업', '등록', '배너', '활성화', '토글', '비활성', '롤링'] },
  // ── 설정 ──
  { id: 'settings-treatment', sectionId: 'settings', type: 'feature', title: '시술 마스터 관리',
    content: '시술명/카테고리/가격대/소요시간/알림주기/활성 여부. 비활성화 시 선택 목록에서 숨김',
    keywords: ['시술', '마스터', '가격', '카테고리', '등록', '시술목록'] },
  { id: 'settings-staff', sectionId: 'settings', type: 'feature', title: '직원 관리',
    content: '이름/이메일/역할(관리자/의사/간호사/직원)/직위/활성. 퇴사 시 비활성화 권장',
    keywords: ['직원', '스태프', '의사', '간호사', '역할', '계정', '퇴사'] },
  { id: 'settings-audit', sectionId: 'settings', type: 'feature', title: '감사 로그',
    content: '모든 관리자 작업(생성/수정/삭제/로그인/내보내기) 이력 추적. 작업유형/사용자/기간 필터',
    keywords: ['감사', '로그', '이력', '추적', '기록', '누가', '변경'] },
  { id: 'settings-clinic', sectionId: 'settings', type: 'feature', title: '병원 정보 설정',
    content: '병원명/전화번호/이메일/주소/카카오 채널/운영시간/점심시간/알림 설정/목표 매출',
    keywords: ['병원', '정보', '설정', '운영시간', '목표', '매출', '점심', '전화번호', '주소'] },
  // ── FAQ ──
  { id: 'faq-app', sectionId: 'faq', type: 'faq', title: '별도 앱을 설치해야 하나요?',
    content: '아니요. 모바일 브라우저에서 접속하면 모든 기능 사용 가능. 홈 화면에 추가하면 앱처럼 사용',
    keywords: ['앱', '설치', '다운로드', '어플'] },
  { id: 'faq-voice', sectionId: 'faq', type: 'faq', title: '음성 인식이 안 돼요',
    content: '마이크 권한 확인(브라우저 자물쇠 > 권한 > 마이크 허용). 인터넷 연결 필요',
    keywords: ['음성', '인식', '안돼', '안되', '마이크', '권한'] },
  { id: 'faq-multi', sectionId: 'faq', type: 'faq', title: '여러 명이 동시에 사용할 수 있나요?',
    content: '네. 각자의 기기에서 로그인하면 동시 사용 가능. 실시간 동기화 지원',
    keywords: ['동시', '접속', '여러명', '같이', '함께', '동기화'] },
  { id: 'faq-backup', sectionId: 'faq', type: 'faq', title: '데이터 백업은 어떻게 하나요?',
    content: '클라우드(Supabase)에 자동 저장. CSV 내보내기 기능으로 수동 백업 가능',
    keywords: ['백업', '저장', '데이터', '보관', '복구', '클라우드'] },
  { id: 'faq-noti-real', sectionId: 'faq', type: 'faq', title: '환자에게 알림이 실제로 발송되나요?',
    content: '네. 알림관리에서 발송 버튼을 누르면 카카오톡 또는 SMS로 실제 전송. 테스트 시 주의',
    keywords: ['발송', '실제', '카카오', 'SMS', '전송', '진짜'] },
  { id: 'faq-delete', sectionId: 'faq', type: 'faq', title: '삭제한 이벤트/팝업은 복구 가능한가요?',
    content: '삭제 후 복구 불가. 삭제 대신 비활성화를 권장',
    keywords: ['삭제', '복구', '되돌리기', '취소', '비활성'] },
  { id: 'faq-goal', sectionId: 'faq', type: 'faq', title: '리포트의 목표 매출은 어디서 설정하나요?',
    content: '설정 > 병원 정보 탭 하단의 "목표 매출" 항목에서 월 목표 금액(원) 입력',
    keywords: ['목표', '매출', '설정', '어디', '위치', '달성율'] },
  { id: 'faq-audit', sectionId: 'faq', type: 'faq', title: '감사 로그에는 어떤 것이 기록되나요?',
...
function removeParticles(text: string): string[] {
  const particles = /[은는이가을를에서도로의와과만요까지부터처럼같이한테에게으로]*$/;
  return text
    .toLowerCase()
    .replace(/[?？!！.。,，]/g, '')
    .split(/\s+/)
...
function searchGuide(query: string): GuideSearchEntry[] {
  ...
  tokens.forEach(t => {
    ...
    Object.values(SYNONYMS).forEach(syns => {
...
const TYPE_LABELS: Record<GuideSearchEntry['type'], string> = {
  scenario: '시나리오', feature: '기능', faq: 'FAQ', tip: '팁', overview: '안내',
};

function SearchResultCard({ entry, onNavigate }: { entry: GuideSearchEntry; onNavigate: (id: string) => void }) {
  const section = TOC_SECTIONS.find(s => s.id === entry.sectionId);
  return (
    <button
      onClick={() => onNavigate(entry.sectionId)}
      className="w-full text-left bg-white border border-[#e5e5e5] rounded-xl p-4 hover:border-[#b4988d] transition-colors cursor-pointer"
    >
      <div className="flex items-center gap-2 mb-2">
        <span className="text-sm">{section?.icon}</span>
        <span className="text-xs font-medium text-[#b4988d]">{section?.label}</span>
        <span className="text-[10px] px-1.5 py-0.5 bg-[#f6f6f6] rounded text-[#8a8a8a]">{TYPE_LABELS[entry.type]}</span>
      </div>
      <p className="text-sm font-medium text-[#6d4e42] mb-1">{entry.title}</p>
      <p className="text-xs text-[#575756] leading-relaxed">{entry.content}</p>
      <p className="text-xs text-[#b4988d] mt-2 font-medium">해당 섹션으로 이동 →</p>
    </button>
  );
}
...
export default function GuidePage() {
  ...
  const searchResults = useMemo(() => searchGuide(searchQuery), [searchQuery]);
      ...
      <div className="bg-white border-2 border-[#b4988d]/20 rounded-2xl p-5 mb-8 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="relative">
          <input
            ...
            aria-label="가이드 검색"
            className="w-full px-4 py-3 pr-10 rounded-xl border border-[#e5e5e5] text-sm text-[#575756] placeholder-[#c0c0c0] focus:outline-none focus:border-[#b4988d] focus:ring-1 focus:ring-[#b4988d]/30"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#c0c0c0] hover:text-[#8a8a8a] cursor-pointer"
              aria-label="검색어 지우기"
            >
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {QUICK_TAGS.map(tag => (
              <button
                ...
                className="px-3 py-1.5 rounded-full text-xs bg-[#f6f6f6] text-[#575756] hover:bg-[#b4988d]/10 hover:text-[#6d4e42] cursor-pointer transition-colors"
              >
                {tag}
              </button>
            ))}
        ...
        {searchQuery && (
          <div className="mt-4">
            {searchResults.length > 0 ? (
              <>
                <p className="text-xs text-[#8a8a8a] mb-3">검색 결과 {searchResults.length}건</p>
                <div className="space-y-2" role="list">
                  {searchResults.map(entry => (
                    <div key={entry.id} role="listitem">
                      <SearchResultCard entry={entry} onNavigate={handleSearchNavigate} />
                    </div>
                  ))}
                </div>
              </>
              ...
              <div className="text-center py-6">
                <p className="text-sm text-[#8a8a8a]">검색 결과가 없습니다.</p>
                <p className="text-xs text-[#c0c0c0] mt-1">다른 키워드로 검색해보세요.</p>
              </div>
            )}
          </div>
        )}
