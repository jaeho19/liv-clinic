      ...
      {!isKo && (
        <AnimatePresence mode="wait">
          {showSuccess && !isFormOpen ? (
            ...
            >
              <div className={`max-w-2xl mx-auto px-3 py-2.5 sm:py-3 ${chatEnabled ? 'mobile-chat-dock' : ''}`}>
                <div className="flex items-center gap-2 sm:gap-3">
                  {chatEnabled && (
                    <button
                      type="button"
                      onClick={handleOpenChat}
                      className="hidden md:flex flex-1 min-h-[48px] bg-primary text-white rounded-lg font-medium text-sm sm:text-base hover:bg-primary/90 active:scale-[0.98] transition-all items-center justify-center gap-2"
                    >
