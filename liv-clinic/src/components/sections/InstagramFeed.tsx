...
function PostCard({ post, index, isLoading }: { post: InstagramPost; index: number; isLoading: boolean }) {
  const t = useTranslations('instagram.ui');
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4">
          <p className="text-white text-sm line-clamp-2">
            {post.caption?.slice(0, 80) || t('viewDetails')}
          ...
          <AnimateOnScroll>
            <div className="mt-8 text-center">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
              >
                <span className="font-medium">{t('showMore')}</span>
        ...
        <AnimateOnScroll>
          <div className="mt-12 text-center">
            ...
            >
              ...
              <span className="font-medium">{t('followCta')}</span>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
