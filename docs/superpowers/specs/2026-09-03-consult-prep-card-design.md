...
[2단계] 서술      "어떻게 보이는 게 제일 싫으세요?"          ← 자유 입력, 어느 언어든
                  **필수** — 비우면 다음으로 못 넘어감 (최소 2자)
                  예시 3개를 회색 placeholder로 보여줌 (입력 부담 완화)
      ▼
[3단계] 시기      "언제까지 필요하세요?"
...
| `reviewed_by` | 검수자. `원장` · `실장` · `원장,실장` 만 허용 | `원장,실장` |
...
1. **고민 분류는 기존 5종 그대로.** `이중턱`은 `sagging`에, `팔자주름`은 `sagging`에, `모공·피부결`은 `texture`에 흡수한다. 분류를 늘리는 판단은 프로토타입을 관계자에게 보인 뒤로 미룬다.
2. **2단계 자유 서술은 필수.** 비우고 다음으로 넘어갈 수 없다. 이 입력이 없으면 카드 1(용어)과 카드 3(질문)이 고민 카드 기본값으로만 채워져 사람마다 결과가 같아지는데, 그러면 이 기능의 존재 이유가 사라진다. 대신 이탈을 줄이기 위해 ① 예시 3개를 회색 placeholder로 보여주고 ② 최소 길이를 2자로만 두어 문턱을 낮춘다.
3. **규칙표 검수는 원장 + 실장 공동.** `reviewed_by`는 자유 문자열이 아니라 `원장` · `실장` · `원장,실장` 세 값만 허용하고, 그 외 값이면 빌드가 실패한다. 오타로 검수 이력이 흐려지는 것을 막는다.

### 다음 차수로 미룬 판단

프로토타입을 관계자에게 보인 뒤 결정한다: 고민 분류 확장 여부 · 결과 카드에 가격대를 넣을지 · 실장 화면 착수 시점.
