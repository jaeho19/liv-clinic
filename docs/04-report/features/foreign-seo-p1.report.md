...
| 항목 | 이유 | 대안 |
|---|---|---|
| 가이드 `published` 전환 | 사장님 검수 전 | `docs/05-handoff/p1-review-checklist.md`대로 검수 → frontmatter 변경 |
| 이벤트 설명 DB 반영 | 승인 필요(프로덕션 DB 쓰기) | 승인 시 `apply-event-2026-09-copy.mjs --commit` 또는 관리자 화면 |
...
1. **가이드 검수**: `docs/05-handoff/p1-review-checklist.md`의 표식(비행 시점·붓기 일수·음주/사우나·예약 변경 규정·공휴일 휴진·모바일 결제·세금 환급). 한 번 답을 주시면 6주제 모두에 반영한다.
2. **상담비 문구**: 사이트 Q&A대로 "1:1 상담 1만원, 당일 시술 시 전액 차감"을 가이드 8편에 넣었다. 외국인·통역 동반 상담에도 같은지 확인.
3. **이벤트**: 포스터 배지 "9월 이벤트 미정/TBA" 의도 여부, "메세지 수신자 해당" 조건을 외국어 설명에 넣을지, 제목 변경 여부, 외국인 이벤트가 적용 여부 → 문서 §"확인이 필요한 것".
4. **fr·ar 로케일**: 언급이 없어 현행 유지했다. 축소·삭제 의향이 있으면 다음 단계.
5. **머지·배포 승인** 및 배포 후 모바일 LCP 측정(4초 초과 시 히어로 영상 모바일 포스터 전환 제안, C5).
